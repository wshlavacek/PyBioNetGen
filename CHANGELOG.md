...
## 0.9.1
Fixed crash when accessing expression data on truncated results (e.g. after stop condition) by replacing private `result._core` access with the public `result.expression_names` and `result.expressions` API. Migrated NFsim session code from private `bngsim._bngsim_core.NfsimSimulator` to the new public `bngsim.NfsimSession` context manager, which handles cleanup automatically and returns `Result` directly from `simulate()`. Simplified `HAS_NFSIM` import to use the public `bngsim` top-level export. No remaining imports from `bngsim._bngsim_core`.

## 0.9.0
Added BNGsim bridge module for in-process ODE, SSA, and NFsim simulation without subprocess calls to BNG2.pl. Integrated BNGsim into the CLI `run` command and `bionetgen.run()` library API. Added steady-state solver and threaded steady-state parameter scanning. Added table functions, SSA/PSA normalization, and BNGsim-integrated NFsim support for `parameter_scan`. Fixed protocol `saveParameters`/`resetParameters` and NFsim concentration propagation. Made atomizer an optional dependency, installable via `pip install bionetgen[atomizer]`. Added pyproject.toml with ruff and mypy configuration. Expanded unit test suite covering bngsim\_bridge, XML parsers, core data structures, simulators, and more. Fixed expression eval namespace collision in bngsim\_bridge where model parameters could shadow math builtins. Promoted codegen failure logging from debug to warning. Made steady-state scan thread pool worker count configurable.

## 0.10.0
**Breaking:** dropped Python 3.8 and 3.9 support; minimum is now Python 3.10. Replaced the deprecated `distutils` stdlib (removed in Python 3.12) — `distutils.spawn.find_executable` is now `shutil.which`, and `distutils.ccompiler` is sourced lazily from `setuptools._distutils` only when the cpy simulator is instantiated. Added `python -m bionetgen` entry point. Avoided spurious network generation for pure NFsim BNGL models and preserved BNGL-declared simulation methods when running under BNGsim. Replayed NFsim concentration changes through the exact-species API and aggregated overrides at the molecule-type level so subsequent simulation segments see the right populations. Reworked release packaging to live in `pyproject.toml` with explicit BioNetGen-asset vendoring via `scripts/vendor_bionetgen_assets.py`, and aligned local and CI checks on `uv`. Tightened error-handling contracts across `xmlparsers`, `structs`, `blocks`, `bngfile`, `csimulator`, `networkparser`, and `gdiff`, replacing ad-hoc `print` warnings and bare exits with `BNGLogger` calls and typed `BNGError` subclasses; added block-dispatch validation and matching test coverage. Smaller cleanups: routed block `__setattr__` warnings through `BNGLogger`, collapsed dead `OrderedDict` import guards, annotated `_raise_unsupported_simulator_type` as `NoReturn`, and rewrote the copy-pasted `Network` class docstring. Taught `FunctionBlockXML` to reconstruct inline-array `tfun([t1,…],[v1,…],idx)` and file-based `TFUN(arg,"file")` calls from BNG2.pl's XML attributes instead of round-tripping its `__TFUN_VAL__` placeholder, which BNG2.pl can't re-parse. Made `_parse_table_functions` join BNGL `\` line continuations so multi-line `tfun()` definitions are recognized. Routed models with `tfun()` to the interpreted ODE RHS in BNGsim (skip codegen) until BNGsim's codegen+tfun runtime is stable. Added `scripts/sync_bng_perl_from_source.py` for refreshing the Perl side of the bundled BNG2.pl from a local upstream checkout when the changes you need have merged but not yet been release-tagged; binaries and `VERSION` are intentionally untouched, and a sibling `assets/BNG_PERL_SOURCE` records the source SHA. Initialized `stdout_str`/`stderr_str` defaults in `BNGCLI.run()` so a BNG2.pl failure no longer raises `UnboundLocalError` and instead surfaces the actual stderr to the caller.

## Unreleased
BNGsim minimum version is now **0.6.0**. PyBioNetGen treats BNGsim as an *optional* dependency by design, but when BNGsim is installed it must be at the minimum version or newer — older releases lack the NFsim global/composite function support (`Result` expression columns) and the `NfsimSession.save_species` / `relative_time=False` APIs that PyBioNetGen's bridge and backend hook depend on. A new runtime version guard in `bionetgen.core.tools.bngsim_bridge` enforces this contract: if BNGsim is not importable the bridge stays silent (the documented subprocess-only path); if BNGsim is importable but older than the minimum, the bridge marks itself unavailable with a descriptive reason, emits a one-time WARNING when `--simulator auto` falls back to subprocess, and raises a precise `BNGSimError` whenever an explicit `--simulator bngsim` (or a BNGsim-required input format like SBML/Antimony) cannot be satisfied. The minimum-version comparison uses `packaging.version.Version` so `0.10.0 > 0.6.0` resolves correctly. Public helpers `is_bngsim_available()` and `get_bngsim_unavailable_reason()` expose the guard's state for CLI surfacing.
The chosen minimum will rise over time as PyBioNetGen takes dependencies on newer BNGsim features; the canonical place to read "what BNGsim does PyBioNetGen need" is the `MINIMUM_BNGSIM_VERSION` constant.

BNGsim `parameter_scan` fast path: a BNGL whose actions are a `generate_network` followed by a single `ode`/`cvode` `parameter_scan` is now driven in-process by BNGsim instead of crossing the backend-hook process/socket/JSON boundary once per scan point. BNG2.pl generates the reaction network once; the new `bngsim_parameter_scan` driver then builds the BNGsim model once, varies the scanned parameter, and re-integrates per point — re-deriving any parameter-dependent initial concentrations itself, since BNGsim freezes those at load time. A 200-point scan that took ~22 s on the backend-hook route (and ~10 s as a BNG2.pl subprocess) now runs in ~2–3 s. The fast path is a pure optimization: anything it cannot handle conservatively (`ssa`/`nf`/`rm`/`pla` scans, `bifurcate`, `par_scan_vals`, `sample_times`, `steady_state`, `reset_conc=>0`, non-trivial action sequences, or a scan action with parser-ambiguous text) declines and falls back to the backend-hook route, and any failure inside the fast path also falls back — correctness never depends on it. Removed the now-moot `TIMEOUT_OVERRIDES` band-aid from `scripts/parity_sweep.py`, and taught `scripts/parity_diff.py` to numeric-diff `.scan` files.

BNGsim backend hook: network-free (`nf`/`rm`) simulation segments now report their time axis starting at 0, matching BNG2.pl/NFsim's "time elapsed since `t_start`" convention. Previously a network-free segment carrying a non-zero `t_start` — e.g. the later stage of a multi-segment protocol — was simulated over `(t_start, t_end)`, which both mislabeled the output time column and shifted any `time()`-dependent rate law onto the wrong interval. Network methods (`ode`/`ssa`/`psa`) are unchanged: they continue to honor `t_start`, as `run_network` does. This `nf`-vs-network difference in `t_start` handling is a longstanding BNG/NFsim behavior (NFsim has no absolute-time concept and reports elapsed time only); it is documented in the README and CLI docs, and tracked upstream as RuleWorld/nfsim#78.